...
Treatment, Day, and the interaction of Treatment*Day are all very significant, and combined explain ~70% of the variance. However, as it turns out the data are heteroscedastic (see below), which may invalidate these results. See test below.
...
These are all significant, which means the data are heteroscedastic with Treatment X Day being closest to not being heterscedastic (i.e. highest p value). PERMANOVA tests are sensitive to unbalanced/heterscedastic data (though it is the most robust of all the possible tests) which can bring to question the validity of these results. On the positive side, the significance is very strong and the heatmap suggests that this is a real biological effect. This is also not uncommon in time-series data sets where there are drastic changes over time. However there is one more thing to consider...

### Confounding variable

Out of curiosity I also tested the forward and reverse Pacbio primers (the indices used for multiplexing) for significance and found that the reverse primers indices used for multiplexing were very significant, which is unusual. Based on the metadata, it looks like the reverse primers are confounded with the treatments. There are 3 primers per antibiotic. This means that they are not evenly spread out across treatments. It's not likely that these have affected your project results, but we can't completely rule it out either.

Reverse_Primer test:

```{r}
set.seed(12345)
adonis2(finalPhyseq.prop.dist.bc ~ Reverse_Primer,
       data = finalPhyseq.meta)
```

Treatment x Day X Reverse Primer
```{r}
set.seed(12345)
adonis2(finalPhyseq.prop.dist.bc ~ Trt_Day_RevP,
       data = finalPhyseq.meta)
```
Treatment x Day x Reverse Primer is very significant, however, the R2 values is only a couple of percentage points higher than the Treatment x Day tests, which means that Reverse_Primer isn't affecting much, if anything.


Reverse Primer tests for heteroscedasticity

Reverse_Primer:
```{r}
beta <- betadisper(finalPhyseq.prop.dist.bc, finalPhyseq.meta$Reverse_Primer)
permutest(beta)
```

Reverse Primer is not heteroscedastic

Treat_Day_RevP:
```{r}
beta <- betadisper(finalPhyseq.prop.dist.bc, finalPhyseq.meta$Trt_Day_RevP)
permutest(beta)
```

Treatment x Day x RevP is NOT heteroscedastic, but it's close.
